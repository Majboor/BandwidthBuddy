...
ALERT_MB_THRESHOLD = 4      # Alert if usage >1MB
RED_BAR_THRESHOLD_MB = 0.5  # 500KB = 0.5MB for red bars

class ProcessMonitorApp(rumps.App):
    def __init__(self):
        super(ProcessMonitorApp, self).__init__("WiFi Monitor", icon=None, menu=["Loading..."])
        self.previous_data = {}
        self.timer = rumps.Timer(self.refresh_menu, 10)  # Auto refresh every 10s
        self.timer.start()
        self.refresh_menu()
    ...
    def usage_bar(self, mb_used, max_mb, bar_length=10):
        if max_mb == 0:
            return ""
        filled = int((mb_used / max_mb) * bar_length)
        filled = min(filled, bar_length)  # avoid overfill
        if mb_used > RED_BAR_THRESHOLD_MB:
    ...
    def refresh_menu(self, _=None):
        ...
        max_mb = live_usage[0]['delta'] if live_usage else 1
        ...
        for item in live_usage[:5]:
            bar = self.usage_bar(item['delta'], max_mb)
            display = f"{item['process']} {bar} ({item['delta']} MB)"
