...
class ProcessMonitorApp(rumps.App):
    ...
    def refresh_menu(self, _=None):
        ...
        if processes:
            ...
            for item in processes:
                usage_in_mb = item['total'] / (1024 * 1024)
                if usage_in_mb > ALERT_MB_THRESHOLD:
                    rumps.notification("High Usage Alert", f"{item['process']} has used {round(usage_in_mb, 2)} MB", "")

            # Top 5 Usage Apps
            self.menu.add("Top 5 Usage Apps")
            for item in processes[:5]:
                usage_in_mb = item['total'] / (1024 * 1024)
                display = f"{item['process']} ({round(usage_in_mb, 2)} MB)"
            ...
            for group_name, items in grouped.items():
                if items:
                    ...
                    for item in items:
                        usage_in_mb = item['total'] / (1024 * 1024)
                        process_display = f"{item['process']} ({round(usage_in_mb, 2)} MB)"
    ...
    def restart_all_apps(self, _):
        ...
        if confirm == 1:
            ...
            # 2. Kill Major System Apps
            os.system("killall Finder Dock SystemUIServer")

            # 3. Kill Browsers and Common Apps
