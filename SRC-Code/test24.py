...
ALERT_MB_THRESHOLD = 200  # 200MB alert
    ...
    def refresh_menu(self, _=None):
        ...
        if processes:
            ...
            for item in processes:
                usage_in_mb = round(item['total'] / 1048576, 2)
                if usage_in_mb > ALERT_MB_THRESHOLD:
                    rumps.notification("High Usage Alert", f"{item['process']} has used {usage_in_mb} MB", "")

            # Top 5 Usage Apps
            self.menu.add("Top 5 Usage Apps")
            for item in processes[:5]:
                usage_in_mb = round(item['total'] / 1048576, 2)
                display = f"{item['process']} ({usage_in_mb} MB)"
            ...
            for group_name, items in grouped.items():
                if items:
                    ...
                    for item in items:
                        usage_in_mb = round(item['total'] / 1048576, 2)
                        process_display = f"{item['process']} ({usage_in_mb} MB)"
